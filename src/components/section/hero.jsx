    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full flex justify-between items-center p-6 z-[100]">
        ...
        <nav className="relative">
          ...
          >
            <span className={`block h-0.5 w-6 bg-white transition-transform ${isNavOpen ? 'rotate-45 translate-y-1.5' : ''}`} />
            <span className={`block h-0.5 w-6 bg-white transition-opacity ${isNavOpen ? 'opacity-0' : 'opacity-100'}`} />
            <span className={`block h-0.5 w-6 bg-white transition-transform ${isNavOpen ? '-rotate-45 -translate-y-1.5' : ''}`} />
          </button>
          ...
          {/* Mobile Nav Panel */}
          {isNavOpen && (
            <div className="md:hidden absolute right-6 top-full mt-2 bg-black/90 rounded-lg p-4 z-50 w-44">
              <ul className="flex flex-col space-y-3">
                <li><a href="/" onClick={() => setIsNavOpen(false)} className="hover:text-yellow-400">Home</a></li>
                <li><a href="/services" onClick={() => setIsNavOpen(false)} className="hover:text-yellow-400">Services</a></li>
                <li><a href="/about" onClick={() => setIsNavOpen(false)} className="hover:text-yellow-400">About Us</a></li>
                <li><a href="/contact" onClick={() => setIsNavOpen(false)} className="hover:text-yellow-400">Contact</a></li>
              </ul>
            </div>
          )}
        </nav>
